        ...
        public static async Task ExportPreviews(int limit = 0)
        {
            while (_requests.Count > limit)
            {
                ...
                for (int i = _requests.Count - 1; i >= 0; i--)
                {
                    ...
                    // check if file is directly in temp folder (means had no dependencies) or in a sub directory (means had dependencies so cannot be deleted if there are more requests still in the queue)
