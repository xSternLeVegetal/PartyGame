...
namespace AssetInventory
{
    public partial class IndexUI : BasicEditorUI
    {
        ...
        {
            _requireAssetTreeRebuild = true;
            if (_tab == 0 && _selectedEntry != null && _selectedEntry.ForeignId == foreignId)
            {
...
#pragma warning restore CS4014
        ...
        private void ReloadLookups()
        {
            ...
            _assetSortOptions = new[] {"Name", "Purchase Date", "Last Update", "Size", "Location", "Hot"};
        ...
        public override void OnGUI()
        {
            ...
            UIStyles.selectedTile.fixedHeight = UIStyles.tile.fixedHeight + UIStyles.tile.margin.top;
            UIStyles.selectedTile.fixedWidth = UIStyles.tile.fixedWidth + UIStyles.tile.margin.left;
...
#pragma warning restore CS4014
        ...
        private void GUILabelWithText(string label, string text, int width = 95, string tooltip = null, bool wrappable = false)
        {
            ...
            EditorGUILayout.LabelField(UIStyles.Content(text, text), wrappable ? EditorStyles.wordWrappedLabel : EditorStyles.label, GUILayout.MaxWidth(UIStyles.INSPECTOR_WIDTH - width - 20), GUILayout.ExpandWidth(false));
            GUILayout.EndHorizontal();
        }
        ...
        private async void LoadTextures(bool firstPageOnly, CancellationToken ct)
        {
            ...
            foreach (AssetInfo info in files)
            {
                ...
                if (info.HasPreview())
                {
                    ...
                    {
                        Debug.LogWarning($"Preview file for {info} does not exist anymore. Marking it missing for recreation.");
                        DBAdapter.DB.Execute("update AssetFile set PreviewState=? where Id=?", AssetFile.PreviewOptions.Redo, info.Id);
                ...
                if (!info.HasPreview())
                {
                    ...
                    if (_staticPreviews.ContainsKey(info.Type))
                    {
                        _contents[idx].image = EditorGUIUtility.IconContent(_staticPreviews[info.Type]).image;
                    }
                    else
                    {
                ...
                if (texture != null && _contents.Length > idx) _contents[idx].image = texture;
            }
        }
