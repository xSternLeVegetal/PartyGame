        ...
        private void CellGUI(Rect cellRect, TreeViewItem<AssetInfo> item, Columns column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case Columns.Version:
                    ...
                    if ((item.Data.AssetSource == Asset.Source.Archive || item.Data.AssetSource == Asset.Source.CustomPackage) && string.IsNullOrWhiteSpace(item.Data.Version))
                    {
                    ...
                    {
                        GUIContent version = new GUIContent(item.Data.AssetSource == Asset.Source.RegistryPackage ? item.Data.InstalledPackageVersion() : item.Data.Version);
                ...
                case Columns.Indexed:
                    ...
                    {
                        Texture indexedIcon = UIStyles.IconContent("Valid", "Installed", "|Indexed").image;
                ...
                case Columns.Name:
                    ...
                    if (item.Data.Id > 0)
                    {
                        ...
                        else if (item.Data.AssetSource == Asset.Source.RegistryPackage)
                        {
                            #if UNITY_2020_1_OR_NEWER
                            Texture folderIcon = EditorGUIUtility.IconContent("d_Package Manager").image;
                            if (folderIcon != null) GUI.DrawTexture(toggleRect, folderIcon);
                            #endif
                        }
                        else if (item.Data.AssetSource == Asset.Source.Archive)
                        {
                            Texture folderIcon = EditorGUIUtility.IconContent("d_FilterByType").image;
                            if (folderIcon != null) GUI.DrawTexture(toggleRect, folderIcon);
                        }
                        else if (item.Data.AssetSource == Asset.Source.Directory)
                        {
