...
namespace AssetInventory
{
    public partial class IndexUI
    {
        ...
        private GUIContent[] _gridSelection;
        private int _gridSelectionMin;
        private int _gridSelectionMax;
        private int _gridSelectionTile;
        private int _gridSelectionCount;
        private int _gridSelectionPackageCount;
        private List<AssetInfo> _gridSelectionItems;
        private long _gridSelectionSize;
        private int _lastGridSelectionTile;
        ...
        private void DrawSearchTab()
        {
            if (_packageFileCount == 0)
            {
                ...
                if (canStillSearch)
                {
                    ...
                    {
                        SetupWizardUI setupUI = SetupWizardUI.ShowWindow();
                    }
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
            ...
            else
            {
                ...
                if (_contents != null && _contents.Length > 0)
                {
                    ...
                    int cells = Mathf.RoundToInt(Mathf.Clamp(Mathf.Floor((position.width - UIStyles.INSPECTOR_WIDTH * inspectorCount - UIStyles.BORDER_WIDTH - 25) / AssetInventory.Config.tileSize), 1, 99));
                    if (cells < 2) cells = 2;

                    // draw contents
                    EditorGUI.BeginChangeCheck();
                    _gridSelectionTile = GUILayout.SelectionGrid(_gridSelectionTile, _contents, cells, UIStyles.tile);
                    Rect gridRect = GUILayoutUtility.GetLastRect();

                    if (_gridSelectionCount > 1)
                    {
                        // draw selection on top if there are more than one selected, otherwise don't for performance
                        GUI.SelectionGrid(gridRect, _gridSelectionTile, _gridSelection, cells, UIStyles.selectedTile);
                    }
                    ...
                    if (EditorGUI.EndChangeCheck() || (_allowLogic && _searchDone))
                    {
                        ...
                        if (_gridSelectionTile >= _files.Count) _gridSelectionTile = 0;
                        _selectedEntry = _files[_gridSelectionTile];
...
#pragma warning restore CS4014
                ...
                else
                {
                    ...
                    bool hasHiddenExtensions = !string.IsNullOrWhiteSpace(AssetInventory.Config.excludedExtensions);
                    ...
                    if (isIndexing || hasHiddenExtensions || hasHiddenPreviews)
                    {
                        ...
                        if (hasHiddenExtensions) GUILayout.Label($"Hidden extensions: {AssetInventory.Config.excludedExtensions}", EditorStyles.centeredGreyMiniLabel);
                ...
                if (!hideDetailsPane && AssetInventory.Config.showSearchDetailsBar)
                {
                    ...
                    if (_gridSelectionCount <= 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (ShowAdvanced()) GUILabelWithText("In Project", _selectedEntry.InProject ? "Yes" : "No");
...
#pragma warning restore CS4014
                    ...
                    else
                    {
                        ...
                        EditorGUILayout.Space();
                        _inspectorScrollPos = GUILayout.BeginScrollView(_inspectorScrollPos, false, false, GUIStyle.none, GUI.skin.verticalScrollbar);
                        GUILabelWithText("Selected", $"{_gridSelectionCount:N0}");
                        if (ShowAdvanced()) GUILabelWithText("Packages", $"{_gridSelectionPackageCount:N0}");
                        GUILabelWithText("Size", EditorUtility.FormatBytes(_gridSelectionSize));

                        EditorGUI.BeginDisabledGroup(_previewInProgress);
                        if (!searchMode && !string.IsNullOrEmpty(_importFolder))
                        {
                            int inProject = _gridSelectionItems.Count(item => item.InProject);
                            if (inProject < _gridSelectionCount)
                            {
                                string command = "Import";
                                if (inProject > 0) command += $" {_gridSelectionCount - inProject} Remaining";

                                GUILabelWithText("Import To", _importFolder);
                                EditorGUILayout.Space();
                                if (GUILayout.Button($"{command} Files")) ImportBulkFiles(_gridSelectionItems);
                            }
                        }

                        if (!searchMode)
                        {
                            if (GUILayout.Button(UIStyles.Content("Open", "Open the files with the assigned system application"))) _gridSelectionItems.ForEach(Open);
                            if (GUILayout.Button(Application.platform == RuntimePlatform.OSXEditor ? "Show in Finder" : "Show in Explorer")) _gridSelectionItems.ForEach(OpenExplorer);
                            EditorGUI.BeginDisabledGroup(_previewInProgress);
                            if (ShowAdvanced() && GUILayout.Button("Recreate Previews")) RecreatePreviews(_gridSelectionItems);
                            EditorGUI.EndDisabledGroup();
                            if (ShowAdvanced()) EditorGUILayout.Space();
                            ...
                            {
                                _gridSelectionItems.ForEach(DeleteFromIndex);
                            }
                        }
                        EditorGUI.EndDisabledGroup();
                        ...
                        // tags
                        DrawAddFileTag(_gridSelectionItems);
                        ...
                        foreach (KeyValuePair<string, Tuple<int, Color>> bulkTag in _assetFileBulkTags)
                        {
                            ...
                            {
                                AssetInventory.RemoveAssetTagAssignment(_gridSelectionItems, bulkTag.Key);
        ...
        private void DrawAddFileTag(List<AssetInfo> assets)
        {
            ...
            if (GUILayout.Button(UIStyles.Content("Add Tag..."), GUILayout.Width(70)))
            {
                ...
                tagUI.Init(TagAssignment.Target.Asset, CalculateBulkSelection);
        ...
        {
            if (_gridSelectionTile >= 0)
            {
                if (!ShowAdvanced() && !Event.current.shift)
                {
                    // regular click, no ctrl/shift
                    _gridSelection.Populate(UIStyles.emptyTileContent);
                    MarkGridSelection();

                    _gridSelectionMin = _gridSelectionTile;
                    _gridSelectionMax = _gridSelectionTile;
                }
                else if (ShowAdvanced())
                {
                    // toggle existing selection
                    _gridSelection[_gridSelectionTile] = _gridSelection[_gridSelectionTile] == UIStyles.selectedTileContent ? UIStyles.emptyTileContent : UIStyles.selectedTileContent;
                }
                else if (Event.current.shift)
                {
                    // shift click - add all between clicks
                    if (_gridSelectionMin != -1 && _gridSelectionMax != -1 && _gridSelectionTile >= _gridSelectionMin && _gridSelectionTile <= _gridSelectionMax)
                    {
                        _gridSelection.Populate(UIStyles.emptyTileContent);
                        _gridSelectionMin = Mathf.Min(_lastGridSelectionTile, _gridSelectionTile);
                        _gridSelectionMax = Math.Max(_lastGridSelectionTile, _gridSelectionTile);
                    }
                    int minI = Mathf.Min(_lastGridSelectionTile, _gridSelectionTile);
                    int maxI = Math.Max(_lastGridSelectionTile, _gridSelectionTile);
                    if (minI < 0) minI = 0;
                    if (maxI < 0) maxI = 0;

                    for (int i = minI; i <= maxI; i++)
                    {
                        _gridSelection[i] = UIStyles.selectedTileContent;
                    }
                }

                _gridSelectionMin = Mathf.Min(_gridSelectionMin, _gridSelectionTile);
        ...
        {
            _gridSelectionItems = _gridSelection
                .Select((item, index) => item == UIStyles.selectedTileContent ? _files[index] : null)
                .Where(item => item != null)
                .ToList();
            _gridSelectionCount = _gridSelectionItems.Count;
            _gridSelectionSize = _gridSelectionItems.Sum(item => item.Size);
            _gridSelectionPackageCount = _gridSelectionItems.GroupBy(item => item.AssetId).Count();

            _assetFileBulkTags.Clear();
            _gridSelectionItems.ForEach(info => info.CheckIfInProject());
            _gridSelectionItems.ForEach(info => info.AssetTags?.ForEach(t =>
            {
        ...
        private async void RecreatePreviews(Asset asset, bool missingOnly, bool retryErroneous, string[] types = null)
        {
            ...
            int created = await new PreviewImporter().RecreatePreviews(asset);
        ...
        private void PerformSearch(bool keepPage = false)
        {
            ...
            _gridSelection = new GUIContent[_contents.Length];
            _gridSelection.Populate(UIStyles.emptyTileContent);
            MarkGridSelection();
            CalculateBulkSelection();
        ...
        {
            if (_gridSelectionTile >= _gridSelection.Length) _gridSelectionTile = 0;
            if (_gridSelection.Length > 0) _gridSelection[_gridSelectionTile] = UIStyles.selectedTileContent;
        }
            ...
            switch (Event.current.type)
            {
                case EventType.MouseDrag:
                    ...
                    if (!_dragging && _selectedEntry != null)
                    {
                        ...
                        if (_gridSelectionCount > 0)
                        {
                            DragAndDrop.SetGenericData("AssetInfo", _gridSelectionItems);
                            DragAndDrop.objectReferences = _gridSelectionItems
