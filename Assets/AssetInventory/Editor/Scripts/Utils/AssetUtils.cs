...
namespace AssetInventory
{
    public static class AssetUtils
    {
        ...
        public static async void LoadTextures(List<AssetInfo> infos, CancellationToken ct)
        {
        ...
        public static async Task LoadPackageTexture(AssetInfo info, bool useCache = true)
        {
            ...
            Texture2D texture;
            if (useCache && PreviewCache.ContainsKey(file))
            {
                texture = PreviewCache[file];
            }
            else
            {
        ...
        {
            if (useCache && PreviewCache.TryGetValue(file, out Texture2D texture)) return texture;
...
#endif
                {
                    Debug.LogError($"Failed to download image from {imageUrl}: {uwr.error}");
                }
                else
                {
                    ...
                    do
                    {
                        ...
                        {
                            Debug.LogWarning($"Could not download image to {targetFile}, retrying: {e.Message}");
